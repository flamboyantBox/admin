...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editArticle(id) {
        console.log("id", id)
        this.$router.push({ path: "/article/publish/" + id});
      },
      updateArticleDelete(id) {
        ...
        this.axios.post("/article/core/recoverOrDeleteArticle", param).then(({ data }) => {
          if (data.code === 200) {
            this.$notify.success({
              title: "成功",
      ...
      // 置顶
      changeTop(article) {
        this.axios.put("/article/core/changeTop", { id: article.id, isTop: article.isTop }).then(({ data }) => {
          if (data.code === 200) {
            this.$notify.success({
              title: "成功",
              message: "置顶成功"
      ...
      listArticles() {
        ...
        this.axios.post("/article/core/articleList", params).then(({ data }) => {
