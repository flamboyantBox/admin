...
<script>
  ...
  export default {
    ...
    methods: {
      listCategories() {
        this.axios.get("/category/core/categoryList").then(({ data }) => {
          this.categoryList = data.data.list;
        });
      },
      listTags() {
        this.axios.get("/tag/core/tagList").then(({ data }) => {
      ...
      uploadCover(response) {
        ...
        if (this.coverCount > 0){
          console.log("coverCount", this.coverCount);
          console.log("articleCover", this.article.articleCover);
          console.log("oldArticleCover", this.oldArticleCover);
          this.axios.delete("/api/oss/file/removeCoverUrl", {
            data:{
              coverUrl: this.oldArticleCover
            }
          })
        }
        this.coverCount = 1;
        this.oldArticleCover = this.article.articleCover
      },
      beforeUpload(file) {
      ...
      searchCategories(keywords, cb) {
        this.axios
          .post("/category/core/search", {keywords: keywords})
          .then(({ data }) => {
            cb(data.data.searchList);
          });
      },
      ...
      searchTags(keywords, cb) {
        this.axios
          .post("/tag/core/search", {keywords: keywords})
          .then(({ data }) => {
            cb(data.data.searchList);
          });
      },
