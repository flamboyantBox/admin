<template>
  <el-card class="main-card">
    ...
    <!-- 新增模态框 -->
    <el-dialog :visible.sync="addModule" width="30%" :close-on-click-modal="false">
    ...
    <!-- 新增模态框 -->
    <el-dialog :visible.sync="addResource" width="30%" :close-on-click-modal="false">
...
<script>
  // import {addOrEditInterface, deleteInterface, getInterfaceList, isPublicInterface} from "../../api/system";
  ...
  export default {
    ...
    methods: {
      listResources() {
        this.axios.get("/resource/core/resourceList",{ keywords: this.keywords }).then(res => {
          this.resourceList = res.data.data.resourceDTO;
      ...
      changeResource(resource) {
        isPublicInterface(resource).then(res => {
          if (res.data.flag) {
            this.$notify.success({
              title: "成功",
      ...
      addOrEditResource() {
        ...
        addOrEditInterface(this.resourceForm).then(res => {
          if (res.data.flag) {
            this.$notify.success({
              title: "成功",
              message: res.data.message
