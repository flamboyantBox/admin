...
<script>
  ...
  export default {
    ...
    methods: {
      openModel(item) {
        if (item) {
          this.albumForm = JSON.parse(item);
      ...
      listAlbums() {
        this.axios
          .post("/photoAlbum/core/albumList", {keywords: this.keywords})
          .then(({ data }) => {
      ...
      addOrEditAlbum() {
        ...
        if (this.albumForm.albumCover == null) {
          this.$message.error("相册封面不能为空");
          return false;
        }

        this.axios
          .post("/photoAlbum/core/saveOrUpdatePhotoAlbum", this.albumForm)
          .then(({ data }) => {
            if (data.code === 200) {
              this.$notify.success({
                title: "成功",
      ...
      uploadCover(response) {
        this.albumForm.albumCover = response.data.url;
      },
      beforeUpload(file) {
      ...
      deleteAlbum() {
        this.axios
          .delete("/photoAlbum/core/removePhotoAlbumById/" + this.albumForm.id)
          .then(({ data }) => {
            if (data.code === 200) {
              this.$notify.success({
                title: "成功",
