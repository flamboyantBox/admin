<template>
  <el-card class="main-card">
    ...
    <div class="album-info">
      ...
      <!-- 相册操作 -->
      <div class="operation">
        <span style="color: #0081ff">压缩图片:</span>
        <el-input class="compressNumber" v-model="UPLOAD_SIZE" size="mini">
          <i slot="suffix" class="compressNumber-i">KB</i>
        </el-input>
        <div class="all-check">
          <el-checkbox
    ...
    />
    <!-- 上传模态框 -->
    <el-dialog :visible.sync="uploadPhoto" width="70%" top="10vh" :close-on-click-modal="false">
    ...
    <!-- 编辑对话框 -->
    <el-dialog :visible.sync="editPhoto" width="30%" :close-on-click-modal="false">
      <div class="dialog-title-container" slot="title">
        修改信息
      </div>
    ...
    <!-- 移动对话框 -->
    <el-dialog :visible.sync="movePhoto" width="30%" :close-on-click-modal="false">
      <div class="dialog-title-container" slot="title">
        移动照片
      </div>
...
<script>
  ...
  export default {
    ...
    data: function() {
      return {
        UPLOAD_SIZE: 102400, // 压缩大小102400KB
        loading: true,
        checkAll: false,
        isIndeterminate: false,
        uploadPhoto: false,
        uploadUrl: 'http://localhost/api/oss/file/upload',
        editPhoto: false,
        movePhoto: false,
    ...
    methods: {
      getAlbumInfo() {
        this.axios
          .get("/photoAlbum/core/getPhotoAlbumBackById/" + this.$route.params.id + "/info")
          .then(({ data }) => {
            this.albumInfo = data.data.albumInfo;
          });
      },
      listAlbums() {
      ...
      listPhotos() {
        this.axios
          .get("/photo/core/photoList", {params: {
              albumId: this.$route.params.id,
              isDelete: 0
            }})
          .then(({ data }) => {
            this.photoList = data.data.list;
      ...
      updatePhoto() {
        ...
        console.log("form", this.photoForm)
        this.axios.put("/photo/core/editPhoto", this.photoForm).then(({ data }) => {
          if (data.code === 200) {
            this.$notify.success({
              title: "成功",
              message: data.message
      ...
      handleRemove(file) {
        this.uploadList.forEach((item, index) => {
          const photo = {}
          photo.photoSrc = file.url
          if (item.url === file.url) {

            this.axios.delete("/api/oss/file/removePhoto", {
              data:{
                photoUrl: photo.photoSrc
              }
            })
              .then(res => {
      ...
      beforeUpload(file) {
        return new Promise(resolve => {
          if (file.size / 1024 < this.UPLOAD_SIZE) {
            resolve(file);
          }
          // 压缩到102400KB,这里的102400就是要压缩的大小,可自定义
          imageConversion
            .compressAccurately(file, this.UPLOAD_SIZE)
            .then(res => {
              resolve(res);
            });
        });
      ...
      updatePhotoDelete(id) {
        ...
        var param = {};
        if (id == null) {
          param = { data: { idList: this.selectPhotoIdList, isDelete: 1 } };
        } else {
          param = { data: { idList: [id], isDelete: 1 } };
        }

        this.axios.delete("/photo/core/deletePhotos", param).then(({ data }) => {
          if (data.code === 200) {
            this.$notify.success({
              title: "成功",
              message: data.message
