...
export default {
  ...
  methods: {
    ...
    listPages() {
      this.axios.get("/page/core/pageList").then( ({data}) => {
    ...
    addOrEditPage() {
      ...
      this.axios.post("/page/core/saveOrUpdatePage", this.pageForum).then(res => {
    ...
    uploadCover(response) {
      this.pageForum.pageCover = response.data.url;
    },
    beforeUpload(file) {
    ...
    deletePage() {
      this.axios.delete("/page/core/removePage/" + this.pageForum.id).then(res => {
        if (res.data.code === 200) {
          this.$notify.success({
            title: "成功",
            message: res.data.message
