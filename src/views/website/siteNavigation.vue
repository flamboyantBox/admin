          ...
          <el-form ref="listQuery" :model="webSiteListQuery">
            <el-col :lg="5" :xl="5">
              <el-form-item label="网站名称" label-width="90px">
                <el-input v-model="keywords" placeholder="网站名称" style="width: 100%;"/>
              </el-form-item>
            </el-col>
            <el-col :lg="5" :xl="5">
              <el-form-item label="网站类型" prop="webSiteTags" label-width="90px">
                <el-select @change="handleFilter()" v-model="tagId" placeholder="请选择网站类型" clearable>
                  <el-option
          ...
          <el-table-column label="操作" align="center" width="190">
            <template slot-scope="{row}">
              ...
              <el-popconfirm
                ...
                @onConfirm="handleDeleteWebSite(row.id)"
              >
          ...
          <el-table-column label="操作" align="center" width="190">
            <template slot-scope="{row}">
              ...
              <el-popconfirm
                title="确定删除吗？"
                style="margin-left:1rem"
                @onConfirm="handleDeleteTags(row.id)"
              >
                <el-button size="mini" type="danger" slot="reference">
                  删除
                </el-button>
  ...
  export default {
    ...
    methods:{
      getAdminSiteNavList() {
        ...
        this.axios.get("/websiteNav/core/websiteNavList", {params : data}).then(res => {
          if (res.data.code === 200) {
            setTimeout(() => {
              this.webSiteLoading =false
            },300)
            this.webSiteList = res.data.data.list
            this.webSiteLoading = true
          }
        })
      },
      getAdminTagsList() {
        this.axios.get("/websiteTags/core/websiteTagList").then(res => {
          ...
          if (res.data.code === 200) {
            ...
            this.siteTagsList = res.data.data.list
            this.tagsOptions = res.data.data.list
          }
        })
      },
      changeTabs(tab) {
      ...
      saveOrUpdateWebSiteData() {
        console.log("data", this.webSiteForm)
        this.axios.post("/websiteNav/core/saveOrUpdateNav", this.webSiteForm).then(res => {
          if (res.data.code === 200) {
            this.getAdminSiteNavList()
      ...
      handleDeleteWebSite(id) {
        this.axios.delete("/websiteNav/core/deleteSiteNav", {data: [id]}).then(res => {
          if (res.data.code === 200) {
            this.getAdminSiteNavList()
            this.$notify.success({
              title: "成功",
      ...
      saveOrUpdateTags() {
        this.axios.post("/websiteTags/core/saveOrUpdateSiteNavTags", this.tagsForm).then(res => {
          if (res.data.code === 200) {
            this.getAdminTagsList()
            this.$notify.success({
              title: "成功",
      ...
      handleDeleteTags(id) {
        this.axios.delete("/websiteTags/core/deleteSiteTags", {data: [id]}).then(res => {
          if (res.data.code === 200) {
            this.getAdminTagsList()
            this.$notify.success({
              title: "成功",
