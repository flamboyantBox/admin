      ...
      <el-table-column label="操作" align="center" width="200">
        <template slot-scope="scope">
          ...
          <el-popconfirm
            ...
            @onConfirm="deleteMenu(scope.row.id)"
          >
...
export default {
  ...
  methods: {
    ...
    listMenus() {
      ...
      this.axios.get("/menu/core/menuList", {params: params}).then(res => {
        console.log("res", res)
        this.menuList = res.data.data.menuDTOList;
        this.loading = false;
      });
    },
    openModel(menu, type) {
    ...
    saveOrUpdateMenu() {
      ...
      this.axios.post("/menu/core/saveOrUpdateMenu", this.menuForm).then(({ data }) => {
    ...
    deleteMenu(id) {
      this.axios.delete("/menu/core/deleteMenu/" + id).then(({ data }) => {
        if (data.code === 200) {
          this.$notify.success({
            title: "成功",
            message: "删除成功"
